...
np.savez('./outputs/pss_exp_var_w2.npz', models_var_sorted, obs_var, np.sqrt(pss_sorted))
...
np.savez('./outputs/EOF_analsys_w2.npz',obs_pc_w2=obs_pc,obs_evec_w2=obs_evec,
         models_pc_w2=models_pc_sorted, models_evec_w2=models_evec_sorted, dx=dx,
         dy=dy)
...
#Todos tienen 2 modos predictibles
# In[11]:

predic_modes = [2, 2, 2, 2, 2]
...
aux1 = np.matmul(np.transpose(obs_pc[0:2, :]), obs_evec[0:2, :])
...
np.savez('./outputs/skill_predic_w2.npz', R_forec_predic_modes, R_forec_residual_modes,
...
#plt.title('Correlation btw Obs vs 2 Obs predictable modes')
